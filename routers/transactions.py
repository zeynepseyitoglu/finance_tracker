...
async def get_transactions(transaction_type: str = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def update_transaction(transaction_id: int, updated_transaction: TransactionCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
